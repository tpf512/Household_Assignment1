...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        # taxes
        par.tax_scale_j = 2.278029 # from Borella et al. (2023), joint, singles: 1.765038
        par.tax_pow_j = 0.0861765 # from Borella et al. (2023), joint, singles: 0.0646416

        par.tax_scale_i = 1.75 # from Borella et al. (2023), individual, singles: 1.765038
        par.tax_pow_i = 0.0646416 # from Borella et al. (2023), individual, singles: 0.0646416
    ...
    def allocate(self):
        ...
        if par.q_6 == True: 
            par.k_grid = np.linspace(0.0,par.k_max-1,par.Nk)
        ...
        sim.tax1 = np.nan + np.zeros(shape)
        sim.tax2 = np.nan + np.zeros(shape)
    ...
    def consumption(self,hours1,hours2,capital1,capital2):
        ...
        if par.joint_tax:
            ...
            tax1 = self.tax_func(income1) 
            tax2 = self.tax_func(income2)
            tax_hh = tax1 + tax2 
    ...
    def tax_func(self,income):
        ...
        if par.joint_tax: 
            rate = 1.0 - par.tax_scale_j*(income**(-par.tax_pow_j))     
        else:
    ...
    def util(self,hours1,hours2,capital1,capital2):
        ...
        if par.weight_u: 
            util_1 = par.mu*((cons*0.5)**(1.0+par.eta) / (1.0+par.eta) - par.rho_1*(hours1)**(1.0+par.gamma) / (1.0+par.gamma))
            util_2 = (1-par.mu)*((cons*0.5)**(1.0+par.eta) / (1.0+par.eta) - par.rho_2*(hours2)**(1.0+par.gamma) / (1.0+par.gamma))
    

        else:
            util_1 = (cons*0.5)**(1.0+par.eta) / (1.0+par.eta) - par.rho_1*(hours1)**(1.0+par.gamma) / (1.0+par.gamma)
            util_2 = (cons*0.5)**(1.0+par.eta) / (1.0+par.eta) - par.rho_2*(hours2)**(1.0+par.gamma) / (1.0+par.gamma) 
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                sim.V[i,t] = interp_2d(par.k_grid,par.k_grid,sol.V[idx_sol],sim.k1[i,t],sim.k2[i,t]) #question 6
