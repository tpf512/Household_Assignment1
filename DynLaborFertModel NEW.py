...
class DynLaborFertModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        par.tau = 0.10 # labor income tax
        par.spouse = 0. # spouse parameter 
        par.theta = 0. #child subisdy parameter 
                ...
                for i_a,assets in enumerate(par.a_grid):
                    for i_k,capital in enumerate(par.k_grid):
                        ...
                        if t==par.T-1: # last period
                            ...
                            # call optimizer
                            hours_min = - (assets - par.theta*kids) / self.wage_func(capital,t) + 1.0e-5 # minimum amout of hours that ensures positive consumption. Add child cost
    ...
    def cons_last(self,hours,assets,capital,kids):
        ...
        spouse = par.spouse*(0.1+0.01*par.T) # add spouse function
        theta = par.theta*0.05*kids # add children cost function
        income = self.wage_func(capital,par.T-1) * hours + spouse - theta 
        cons = assets + income 
        return cons
    ...
    def value_of_choice(self,cons,hours,assets,capital,kids,t):
        ...
        spouse = par.spouse*(0.1+0.01*t)
        theta = par.theta*0.05*kids # add children cost function
        income = self.wage_func(capital,t) * hours + spouse - theta # add spouse function
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                if t<par.simT-1:
                    ...
                    #married = 0 
                    #if ((sim.draws_uniform[i,t] <= par.p_spouse) & (sim.s[i,t]<(par.Ns-1))):
                        #married = 1
                        #sim.a[i,t+1] += par.spouse*(0.1+0.1*t)
                    #sim.s[i,t+1] = sim.s[i,t] + married
                    ...
                    sim.n[i,t+1] = sim.n[i,t] + birth
                        ...
                        #if ((sim.draws_uniform[i,t] <= par.p_birth) & (sim.n[i,t]<(par.Nn-1))):
                                #birth = 1
                                #sim.a[i,t+1] += par.theta*0.05
                        #im.n[i,t+1] = sim.n[i,t] + birth
